...
from apps.coach import utils
from apps.catalog.models import SubCategory, Exercise ,Category, CategoryLevel
from apps.coach import models
from . import variation_horizontal



## Training Express
def training_express(user):
     
    

    uper="Upper"
    low="Lower"
    #user_nivel= models.NivelUser.objects.filter(nivel=user).first()
    equip = models.CategorySelectionNivelUser.objects.filter(nivel_user=user).values("id","equip")
    equip_user=[]
    for e in equip:
        if e["equip"]:
            equip_user.append(e["equip"])
    equip_exercises  = utils.filter_equipment(equip_user)
    # Select id of Category 
    category_selections=[]
    base = models.Category.objects.filter(name= "Base").values("id").first()
    category_selections.append(base["id"])
   
    myo = models.BlockNivel.objects.filter(block__type_exercise="Myofascial").filter(nivel__name="Express").values("id").first()
    mob = models.BlockNivel.objects.filter(block__type_exercise="Mobility").filter(nivel__name="Express").values("id").first()
    skil = models.BlockNivel.objects.filter(block__type_exercise="Skills").filter(nivel__name="Express").values("id").first()
    plyo = models.BlockNivel.objects.filter(block__type_exercise="Plyo").filter(nivel__name="Express").values("id").first()
    streng_primary = models.BlockNivel.objects.filter(block__type_exercise="Primary Strength").filter(nivel__name="Express").values("id").first()
    streng_secundary = models.BlockNivel.objects.filter(block__type_exercise="Secondary Strength").filter(nivel__name="Express").values("id").first()
    esd = models.BlockNivel.objects.filter(block__type_exercise="Esd").filter(nivel__name="Express").values("id").first()
    recovery = models.BlockNivel.objects.filter(block__type_exercise="Recovery").filter(nivel__name="Express").values("id").first()
    activation = models.BlockNivel.objects.filter(block__type_exercise="Activation").filter(nivel__name="Express").values("id").first()

    
    # Quantity of exercises and type

    exercise_type_quantity = models.CategorySelection.objects.filter(name_id__in=category_selections,block_nivel__nivel__name= "Express").values("block_nivel",'quantity_exercise')
